#!/usr/bin/env python
# based on https://github.com/nikagra/python-noise/blob/master/noise.py
import random, math
        ...
        for i in range(self.imageSize):
            for j in range(self.imageSize):
                ...
                if min is None or min > value:
                    min = value
        # pow \in (0, +\infty). if pow < 1, the color is more likely to be white; otherwise more likely to be black.
        for i in range(self.imageSize):
            for j in range(self.imageSize):
                v = (noise[i, j] - min) / (max - min)  # [0, 1]
                pow = 0.2
                self.img[i, j] = (int)((v ** pow) * 255)
